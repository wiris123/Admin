...
public class ConnectDAO {
	...
	public int todayVisitor() {
		int today = 0;
		String query = "select count(to_char(visit_date, 'YY-MM-dd')) from visitor group by to_char(visit_date, 'YY-MM-dd')  having to_char(visit_date, 'YY-MM-dd') =  to_char(sysdate, 'YY-MM-dd')";
		try {
			psmt = con.prepareStatement(query);
			rs = psmt.executeQuery();
			if (rs.next()) {
	...
	public int yesterdayVisitor() {
		int yesterday = 0;
		String query = "select count(to_char(visit_date, 'YY-MM-dd')) from visitor group by to_char(visit_date, 'YY-MM-dd')  having to_char(visit_date, 'YY-MM-dd') =  to_char(sysdate-1, 'YY-MM-dd')";
		try {
			psmt = con.prepareStatement(query);
			rs = psmt.executeQuery();
			if (rs.next()) {
	...
	//한달 평균구하기
	public VisitorValDTO monthly() {
		VisitorValDTO mon= new VisitorValDTO();
		//혹시 없을경우를 위한 오류처리용

		String query = "select sum(count(to_char(visit_date, 'YY-MM-dd'))), avg(count(to_char(visit_date, 'YY-MM-dd'))) from visitor group by to_char(visit_date, 'YY-MM-dd')  having to_char(visit_date, 'YY-MM-dd') between add_months(sysdate,-1) and to_char(sysdate, 'YY-MM-dd')";
		try {
			psmt = con.prepareStatement(query);
			rs = psmt.executeQuery();
			if(rs.next()) {
				mon.setTot(rs.getInt(1));
				mon.setAvg(rs.getDouble(2));
			}
		}
		catch(Exception e) {
			e.printStackTrace();
